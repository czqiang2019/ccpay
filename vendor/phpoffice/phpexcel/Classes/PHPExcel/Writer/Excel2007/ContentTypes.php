...
class PHPExcel_Writer_Excel2007_ContentTypes extends PHPExcel_Writer_Excel2007_WriterPart
{
	 ...
	 * @param	boolean		$includeCharts	Flag indicating if we should include drawing details for echarts
	...
	public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = FALSE)
	{
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			for ($i = 0; $i < $sheetCount; ++$i) {
				...
				//	We need a drawing relationship for the worksheet if we have either drawings or echarts
				...
				//	If we have echarts, then we need a chart relationship for every individual chart
				if ($chartCount > 0) {
					for ($c = 0; $c < $chartCount; ++$c) {
						$this->_writeOverrideContentType(
							$objWriter, '/xl/echarts/chart' . $chart++ . '.xml', 'application/vnd.openxmlformats-officedocument.drawingml.chart+xml'
						);
					}
				}
			}
